...
export default function DocumentUpload({ applicationId, onUploadComplete, presetType }) {
  ...
  useEffect(() => { fetchDocuments(); }, [applicationId]);
  ...
  return (
    <div>
      <p style={S.formSectionTitle}>Documents</p>

      {error && (
        <div style={{ padding: '10px 14px', borderRadius: '8px', background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca', fontSize: '13px', marginBottom: '14px' }}>
          {error}
        </div>
      )}
      {success && (
        <div style={{ padding: '10px 14px', borderRadius: '8px', background: '#f0fdf4', color: '#16a34a', border: '1px solid #bbf7d0', fontSize: '13px', marginBottom: '14px' }}>
          {success}
        </div>
      )}
      ...
      <div style={{
        ...
        borderColor: selectedFile ? '#86efac' : '#e2e8f0',
      }}>
      ...
      {/* Documents list */}
      {loading ? (
        <p style={{ color: '#94a3b8', fontSize: '13px' }}>Loading documents...</p>
      ) : documents.length === 0 ? (
        <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {documents.map(doc => {
            ...
            const iconStyle = iconColors[icon] || iconColors.FILE;
            return (
              <div key={doc.id} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ margin: 0, fontSize: '13.5px', fontWeight: '500', color: '#0f172a', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {doc.file_name}
                  </p>
                  <p style={{ margin: '2px 0 0', fontSize: '11px', color: '#94a3b8' }}>
                    {docTypeLabel(doc.doc_type)} · {new Date(doc.uploaded_at).toLocaleDateString('en-ZA')}
                  </p>
                </div>
                ...
                <div style={{ display: 'flex', gap: '12px', flexShrink: 0 }}>
                  <button
                    ...
                    style={{ background: 'none', border: 'none', color: '#2563eb', fontSize: '12px', cursor: 'pointer', fontFamily: 'DM Sans', fontWeight: '600', padding: 0 }}
                  >
                    Download
                  </button>
                  <button
